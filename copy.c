/*
	Name: Kevin C. Bell, Nicholas J. Bravata, Thomas J. Jury        Class: CPS 360
        Section: 22362460                                               Assignment: 06
        Due: March 14, 2019                                             Started: March 9, 2019
        Credit: 10 points.

	Problem: Write a program that copies the given files to the specified destination directory
	using file system call interface (open(), close(), read(), write(), stat() etc.). The program
	...
	not (isvalid()), and then the other part loops through each given paths and copies if it is a valid file for
	copying (copyingfiles()). isvalid() uses isdir() and isregular() to determine if a file path is valid
	for a copy, if it is it returns a 1 to copyingfiles() letting it know it can copy it, but if it isn't
	it prints out the reason it's not valid and return 1 so copying files doesn't try to copy it. copyingfiles()
	uses buildpath() to know where to put the copy it then lets isvalid() check the file if it is valid, it
	sends the original's path and the copy's path to makecp() for it to copy which also can cancel if there
	is any errors, once copied it loops through the rest of the other files. This is ends once copyingfiles()
	have gone through every file and it returns a 0 if no copies were made ending the program in an EXIT_FAILURE
	and 1 (EXIT_SUCCESS) otherwise.

	Data-structure used: A one-dimensional array for the command line arguments.

	Accessing functions for the data structure: Standard C functions for accessing arrays and structs.

	Errors handled: The program checks if each file path and it's copy path is valid and if the destination path
	is valid, it also checks if the file is already in the place it's trying to be copied too. The program also
	checks if there are errors during the copy process to stop it from making a bad file.

	Limitations: As the file gets bigger the speed in which it gets copied gets slower. Can't copy non-regular files.

	Acknowledgment: We wrote the program together but we had a lot of help from our classmates throughout the making of it.
*/
...
int makecp (char *srcpath, char *dstpath)
{
   char bufs[2048]; int copy, fd, rd;

   if ((fd = open(srcpath, O_RDONLY)) < 0 || (copy = open(dstpath, O_CREAT | O_WRONLY, 0644)) < 0)
   {
      fprintf(stderr, "%-5s: error when accessing file\n", srcpath);
      return 0;
   }
