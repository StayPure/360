/*
        ...
        Problem: Write a program that simulates a dump of a memory block in base 16 digits and ASCII.
        The program will take in a file, a start point, and a end point only dumping the memory
        locations from the start to the end point. The program will takes these specific inputs from
        the command-line and check to make use they are correct. The progam will check if the memory
        address given for the end point is greater than the last memory address and if so adjust to
        the end point to the last memory address. The program needs print out in a specific format
        and the program must check to make sure it doesn't try to print unprintable ASCII codes.
